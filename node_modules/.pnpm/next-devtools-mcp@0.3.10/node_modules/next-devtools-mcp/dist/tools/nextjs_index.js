import { z } from "zod";
import { getAllAvailableServers, listNextJsTools, detectProtocol, MCP_HOST, } from "../_internal/nextjs-runtime-manager.js";
export const inputSchema = {
    port: z
        .union([z.string(), z.number()])
        .transform((val) => (typeof val === "string" ? parseInt(val, 10) : val))
        .optional()
        .describe("Optional port number to probe directly. Use this when auto-discovery fails - ask the user which port their Next.js dev server is running on."),
};
...
async function probeAndListTools(port) {
    try {
    ...
    try {
        ...
        if (servers.length === 0) {
            return JSON.stringify({
                ...
                ai_instruction: "IMPORTANT: Server auto-discovery may not work on all operating systems or network configurations. Please ask the user: 'What port is your Next.js dev server running on?'. Once you have the port number, call this tool again with the 'port' parameter set to the user-provided port.",
                servers: [],
            });
        }
        // Get tools for each server
        const serversWithTools = await Promise.all(servers.map(async (s) => {
        ...
        return JSON.stringify({
            ...
            message: `Found ${serversWithTools.length} Next.js server${serversWithTools.length === 1 ? "" : "s"} with MCP enabled`,
        });
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
