 ...
 */
function getFetchOptions(protocol) {
    const allowInsecure = protocol === "https" && process.env.NODE_TLS_REJECT_UNAUTHORIZED === "0";
    if (protocol !== "https" || !allowInsecure)
        return {};
 ...
 */
async function probeMCPEndpoint(port, protocol, timeoutMs = 500) {
    try {
 ...
 */
async function getPortsViaSs(pid) {
    try {
 ...
 */
async function getPortsViaNetstat(pid) {
    try {
...
async function findNextJsServers() {
    try {
        const servers = [];
        ...
        if (process.platform === "win32") {
            ...
            const nodeProcesses = await findProcess("name", "node", false).catch(() => []);
            ...
            const nextServerProcesses = await findProcess("name", "next-server", true).catch(() => []);
...
async function makeNextJsMCPRequest(port, method, params = {}) {
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 404) {
                throw new Error(`MCP endpoint not found. Next.js MCP support requires Next.js 16+. ` +
        ...
        if (error instanceof TypeError && error.message.includes("fetch failed")) {
            throw new Error(`Cannot connect to Next.js dev server on port ${port}. ` +
...
export async function listNextJsTools(port) {
    try {
...
export async function callNextJsTool(port, toolName, args) {
    try {
 ...
 */
async function discoverViaPortProbing() {
    const servers = [];
...
export async function getAllAvailableServers() {
    ...
    const verifiedServers = await Promise.all(newServers.map(async (server) => {
