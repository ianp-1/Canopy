...
let browserEvalConnection = null;
/**
 * Check if playwright-mcp is installed
 */
async function isPlaywrightMCPInstalled() {
    try {
 ...
 */
export async function ensureBrowserEvalMCP() {
...
export async function startBrowserEvalMCP(options) {
    ...
    const args = ["@playwright/mcp@latest", "--image-responses", "omit"];
    if (options?.browser) {
 ...
 */
export function getBrowserEvalConnection() {
    return browserEvalConnection;
}
/**
 * Stop playwright-mcp server and cleanup
 */
export async function stopBrowserEvalMCP() {
    if (!browserEvalConnection) {
        return;
    }
