...
export async function POST(request: NextRequest) {
  try {
    ...
    const { latitude, longitude, farm_size_hectares, crop_type, coverage_xrp } = body
    ...
    if (
      ...
      return NextResponse.json(
        { error: 'Missing required fields: latitude, longitude, crop_type, coverage_xrp' },
        { status: 400 },
      )
    }
